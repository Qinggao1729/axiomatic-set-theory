...
lemma zero_ω_mem_ω : zero_ω ∈ ω := by
  change Set.Empty ∈ ω
  exact thm_4B_ω_inductive.left

private noncomputable def OmegaSuccessorGraph : Set :=
  GraphOn ω (fun n => n⁺)

private lemma ω_successor_graph_maps_into : MapsInto OmegaSuccessorGraph ω ω := by
  unfold OmegaSuccessorGraph
  apply GraphOn.mapsInto
  intro n hnω
  exact thm_4B_ω_inductive.right n hnω

private lemma recursion_theorem_on_ω_successor (a : Set) :
...
private noncomputable def NatAddRel (m : Set) (hmω : m ∈ ω) : Set :=
  Classical.choose (ExistsUnique.exists (recursion_theorem_on_ω_successor m hmω))

private lemma NatAddRel.solution (m : Set) (hmω : m ∈ ω) :
    RecursionSolution (NatAddRel m hmω) ω m OmegaSuccessorGraph :=
  Classical.choose_spec (ExistsUnique.exists (recursion_theorem_on_ω_successor m hmω))

private lemma ω_successor_graph_pair (x y : Set) :
    OrderedPair x y ∈ OmegaSuccessorGraph ↔ x ∈ ω ∧ y = x⁺ := by
  unfold OmegaSuccessorGraph
  exact GraphOn.Pair.Spec (fun n hnω => thm_4B_ω_inductive.right n hnω)

private lemma nat_add_rel_zero (m : Set) (hmω : m ∈ ω) :
    OrderedPair zero_ω m ∈ NatAddRel m hmω :=
  (NatAddRel.solution m hmω).2.1

private lemma nat_add_rel_step (m n x y : Set) (hmω : m ∈ ω) :
    n ∈ ω →
...
private lemma nat_add_rel_value_exists_unique (m n : Set) (hmω : m ∈ ω) (hnω : n ∈ ω) :
...
private lemma nat_add_spec (m n : Set) (hmω : m ∈ ω) (hnω : n ∈ ω) :
...
/- [Enderton, Theorem 4I (closure corollary for addition), pp.79-80] -/
theorem thm_4I_add_closed (m n : Set) : m ∈ ω → n ∈ ω → m + n ∈ ω := by
  intro hmω hnω
  rcases (NatAddRel.solution m hmω).1 with ⟨_, _, hRanSub⟩
  have hInRan : m + n ∈ ran (NatAddRel m hmω) :=
    (Range.Spec).2 ⟨n, nat_add_spec m n hmω hnω⟩
  exact hRanSub (m + n) hInRan

abbrev nat_add_closed := thm_4I_add_closed

/- [Enderton, Theorem 4I (A1), p.80] -/
theorem thm_4I_A1_add_zero (m : Set) : m ∈ ω → m + zero_ω = m := by
  intro hmω
  ...
  apply function_value_unique (NatAddRel m hmω) zero_ω (m + zero_ω) m
  · exact hFun
  · exact hPairAdd
  · exact hPairZero

abbrev nat_add_zero := thm_4I_A1_add_zero

/- [Enderton, Theorem 4I (A2), p.80] -/
theorem thm_4I_A2_add_succ (m n : Set) : m ∈ ω → n ∈ ω → m + n⁺ = (m + n)⁺ := by
  intro hmω hnω
  have hnSuccω : n⁺ ∈ ω := thm_4B_ω_inductive.right n hnω
  have hPairN : OrderedPair n (m + n) ∈ NatAddRel m hmω :=
    nat_add_spec m n hmω hnω
  have hGraph : OrderedPair (m + n) ((m + n)⁺) ∈ OmegaSuccessorGraph :=
    (ω_successor_graph_pair (m + n) ((m + n)⁺)).2 ⟨nat_add_closed m n hmω hnω, rfl⟩
  have hPairSuccStep : OrderedPair (n⁺) ((m + n)⁺) ∈ NatAddRel m hmω :=
    (nat_add_rel_step m n (m + n) ((m + n)⁺) hmω hnω hPairN).2 hGraph
  have hPairSuccVal : OrderedPair (n⁺) (m + n⁺) ∈ NatAddRel m hmω :=
    nat_add_spec m (n⁺) hmω hnSuccω
  have hFun : IsFunction (NatAddRel m hmω) := (NatAddRel.solution m hmω).1.1
  apply function_value_unique (NatAddRel m hmω) (n⁺) (m + n⁺) ((m + n)⁺)
  · exact hFun
  · exact hPairSuccVal
  · exact hPairSuccStep

private noncomputable def OmegaAddRightGraph (m : Set) : Set :=
  GraphOn ω (fun x => x + m)

private lemma ω_add_right_graph_maps_into (m : Set) :
...
private lemma recursion_theorem_on_ω_add_right (m : Set) :
...
private noncomputable def NatMulRel (m : Set) (hmω : m ∈ ω) : Set :=
  Classical.choose (ExistsUnique.exists (recursion_theorem_on_ω_add_right m hmω))

private lemma NatMulRel.solution (m : Set) (hmω : m ∈ ω) :
    RecursionSolution (NatMulRel m hmω) ω zero_ω (OmegaAddRightGraph m) :=
  Classical.choose_spec (ExistsUnique.exists (recursion_theorem_on_ω_add_right m hmω))

private lemma ω_add_right_graph_pair (m x y : Set) (hmω : m ∈ ω) :
    OrderedPair x y ∈ OmegaAddRightGraph m ↔ x ∈ ω ∧ y = x + m := by
  unfold OmegaAddRightGraph
  exact GraphOn.Pair.Spec (fun t htω => nat_add_closed t m htω hmω)

private lemma nat_mul_rel_zero (m : Set) (hmω : m ∈ ω) :
    OrderedPair zero_ω zero_ω ∈ NatMulRel m hmω :=
  (NatMulRel.solution m hmω).2.1

private lemma nat_mul_rel_step (m n x y : Set) (hmω : m ∈ ω) :
    n ∈ ω →
...
private lemma nat_mul_rel_value_exists_unique (m n : Set) (hmω : m ∈ ω) (hnω : n ∈ ω) :
...
private noncomputable def NatMulCanon (m n : Set) : Set :=
  letI : Decidable (m ∈ ω) := Classical.decPred (fun x : Set => x ∈ ω) m
  letI : Decidable (n ∈ ω) := Classical.decPred (fun x : Set => x ∈ ω) n
  if hmω : m ∈ ω then
    if hnω : n ∈ ω then
...
private lemma nat_mul_canon_spec (m n : Set) (hmω : m ∈ ω) (hnω : n ∈ ω) :
...
private lemma nat_mul_canon_closed (m n : Set) :
    m ∈ ω → n ∈ ω → NatMulCanon m n ∈ ω := by
  intro hmω hnω
  rcases (NatMulRel.solution m hmω).1 with ⟨_, _, hRanSub⟩
  have hInRan : NatMulCanon m n ∈ ran (NatMulRel m hmω) :=
    (Range.Spec).2 ⟨n, nat_mul_canon_spec m n hmω hnω⟩
  exact hRanSub (NatMulCanon m n) hInRan

private lemma nat_mul_canon_zero (m : Set) :
  ...
  apply function_value_unique (NatMulRel m hmω) zero_ω (NatMulCanon m zero_ω) zero_ω
  · exact hFun
  · exact hPairMul
  · exact hPairZero

private lemma nat_mul_canon_succ (m n : Set) :
  ...
  have hGraph : OrderedPair (NatMulCanon m n) ((NatMulCanon m n) + m) ∈ OmegaAddRightGraph m :=
    (ω_add_right_graph_pair m (NatMulCanon m n) ((NatMulCanon m n) + m) hmω).2
      ⟨nat_mul_canon_closed m n hmω hnω, rfl⟩
  have hPairSuccStep : OrderedPair (n⁺) ((NatMulCanon m n) + m) ∈ NatMulRel m hmω :=
    (nat_mul_rel_step m n (NatMulCanon m n) ((NatMulCanon m n) + m) hmω hnω hPairN).2 hGraph
  have hPairSuccVal : OrderedPair (n⁺) (NatMulCanon m (n⁺)) ∈ NatMulRel m hmω :=
    nat_mul_canon_spec m (n⁺) hmω hnSuccω
  have hFun : IsFunction (NatMulRel m hmω) := (NatMulRel.solution m hmω).1.1
  apply function_value_unique (NatMulRel m hmω) (n⁺) (NatMulCanon m (n⁺)) ((NatMulCanon m n) + m)
  · exact hFun
  · exact hPairSuccVal
  · exact hPairSuccStep
...
abbrev nat_add_succ := thm_4I_A2_add_succ

/- [Enderton, Theorem 4J (closure corollary for multiplication), pp.80-81] -/
theorem thm_4J_mul_closed (m n : Set) : m ∈ ω → n ∈ ω → m * n ∈ ω := by
  intro hmω hnω
  change NatMul m n ∈ ω
  simpa [NatMul] using nat_mul_canon_closed m n hmω hnω

abbrev nat_mul_closed := thm_4J_mul_closed

/- [Enderton, Theorem 4J (M1), p.81] -/
theorem thm_4J_M1_mul_zero (m : Set) : m ∈ ω → m * zero_ω = zero_ω := by
  intro hmω
  change NatMul m zero_ω = zero_ω
  simpa [NatMul] using nat_mul_canon_zero m hmω

abbrev nat_mul_zero := thm_4J_M1_mul_zero

/- [Enderton, Theorem 4J (M2), p.81] -/
theorem thm_4J_M2_mul_succ (m n : Set) : m ∈ ω → n ∈ ω → m * n⁺ = (m * n) + m := by
  intro hmω hnω
  change NatMul m (n⁺) = (NatMul m n) + m
  simpa [NatMul] using nat_mul_canon_succ m n hmω hnω

private noncomputable def OmegaMulRightGraph (m : Set) : Set :=
  GraphOn ω (fun x => x * m)

private lemma ω_mul_right_graph_maps_into (m : Set) :
...
lemma one_ω_mem_ω : one_ω ∈ ω := by
  simpa [one_ω, zero_ω] using thm_4B_ω_inductive.right Set.Empty thm_4B_ω_inductive.left

private lemma recursion_theorem_on_ω_mul_right (m : Set) :
...
private noncomputable def NatPowRel (m : Set) (hmω : m ∈ ω) : Set :=
  Classical.choose (ExistsUnique.exists (recursion_theorem_on_ω_mul_right m hmω))

private lemma NatPowRel.solution (m : Set) (hmω : m ∈ ω) :
    RecursionSolution (NatPowRel m hmω) ω one_ω (OmegaMulRightGraph m) :=
  Classical.choose_spec (ExistsUnique.exists (recursion_theorem_on_ω_mul_right m hmω))

private lemma ω_mul_right_graph_pair (m x y : Set) (hmω : m ∈ ω) :
    OrderedPair x y ∈ OmegaMulRightGraph m ↔ x ∈ ω ∧ y = x * m := by
  unfold OmegaMulRightGraph
  exact GraphOn.Pair.Spec (fun t htω => nat_mul_closed t m htω hmω)

private lemma nat_pow_rel_zero (m : Set) (hmω : m ∈ ω) :
    OrderedPair zero_ω one_ω ∈ NatPowRel m hmω :=
  (NatPowRel.solution m hmω).2.1

private lemma nat_pow_rel_step (m n x y : Set) (hmω : m ∈ ω) :
    n ∈ ω →
...
private lemma nat_pow_rel_value_exists_unique (m n : Set) (hmω : m ∈ ω) (hnω : n ∈ ω) :
...
private noncomputable def NatPowCanon (m n : Set) : Set :=
  letI : Decidable (m ∈ ω) := Classical.decPred (fun x : Set => x ∈ ω) m
  letI : Decidable (n ∈ ω) := Classical.decPred (fun x : Set => x ∈ ω) n
  if hmω : m ∈ ω then
    if hnω : n ∈ ω then
...
private lemma nat_pow_canon_spec (m n : Set) (hmω : m ∈ ω) (hnω : n ∈ ω) :
...
private lemma nat_pow_canon_closed (m n : Set) :
    m ∈ ω → n ∈ ω → NatPowCanon m n ∈ ω := by
  intro hmω hnω
  rcases (NatPowRel.solution m hmω).1 with ⟨_, _, hRanSub⟩
  have hInRan : NatPowCanon m n ∈ ran (NatPowRel m hmω) :=
    (Range.Spec).2 ⟨n, nat_pow_canon_spec m n hmω hnω⟩
  exact hRanSub (NatPowCanon m n) hInRan

private lemma nat_pow_canon_zero (m : Set) :
  ...
  apply function_value_unique (NatPowRel m hmω) zero_ω (NatPowCanon m zero_ω) one_ω
  · exact hFun
  · exact hPairPow
  · exact hPairZero

private lemma nat_pow_canon_succ (m n : Set) :
  ...
  have hGraph : OrderedPair (NatPowCanon m n) ((NatPowCanon m n) * m) ∈ OmegaMulRightGraph m :=
    (ω_mul_right_graph_pair m (NatPowCanon m n) ((NatPowCanon m n) * m) hmω).2
      ⟨nat_pow_canon_closed m n hmω hnω, rfl⟩
  have hPairSuccStep : OrderedPair (n⁺) ((NatPowCanon m n) * m) ∈ NatPowRel m hmω :=
    (nat_pow_rel_step m n (NatPowCanon m n) ((NatPowCanon m n) * m) hmω hnω hPairN).2 hGraph
  have hPairSuccVal : OrderedPair (n⁺) (NatPowCanon m (n⁺)) ∈ NatPowRel m hmω :=
    nat_pow_canon_spec m (n⁺) hmω hnSuccω
  have hFun : IsFunction (NatPowRel m hmω) := (NatPowRel.solution m hmω).1.1
  apply function_value_unique (NatPowRel m hmω) (n⁺) (NatPowCanon m (n⁺)) ((NatPowCanon m n) * m)
  · exact hFun
  · exact hPairSuccVal
  · exact hPairSuccStep
...
abbrev nat_mul_succ := thm_4J_M2_mul_succ

/- [Enderton, Theorem 4K (component: left additive identity), pp.82-83] -/
theorem thm_4K_zero_add (n : Set) :
    n ∈ ω → zero_ω + n = n := by
  intro hnω
...
abbrev nat_zero_add := thm_4K_zero_add

/- [Enderton, Theorem 4K (component: left successor-add recursion), pp.82-83] -/
theorem thm_4K_succ_add (m n : Set) :
    m ∈ ω → n ∈ ω → m⁺ + n = (m + n)⁺ := by
  intro hmω hnω
  have hmSuccω : m⁺ ∈ ω := thm_4B_ω_inductive.right m hmω
...
abbrev nat_succ_add := thm_4K_succ_add

/- [Enderton, Theorem 4K (component: commutativity of addition), pp.82-83] -/
theorem thm_4K_add_comm (a b : Set) :
  a ∈ ω → b ∈ ω → a + b = b + a := by
  intro haω hbω
...
abbrev nat_add_comm := thm_4K_add_comm

/- [Enderton, Theorem 4K (component: associativity of addition), pp.82-83] -/
theorem thm_4K_add_assoc (a b c : Set) :
  ...
  · exact hcω
abbrev nat_add_assoc := thm_4K_add_assoc

/- [Enderton, Theorem 4K (component: left distributivity), pp.82-83] -/
theorem thm_4K_left_distrib (a b c : Set) :
...
abbrev nat_left_distrib := thm_4K_left_distrib

/- [Enderton, Theorem 4K (component: associativity of multiplication), pp.82-83] -/
theorem thm_4K_mul_assoc (a b c : Set) :
  a ∈ ω → b ∈ ω → c ∈ ω → (a * b) * c = a * (b * c) := by
  intro haω hbω hcω
...
abbrev nat_mul_assoc := thm_4K_mul_assoc

/- [Enderton, Theorem 4K (component: left multiplicative zero), pp.82-83] -/
theorem thm_4K_zero_mul (n : Set) :
...
abbrev nat_zero_mul := thm_4K_zero_mul

/- [Enderton, Theorem 4K (component: left multiplication successor recursion), pp.82-83] -/
theorem thm_4K_mul_succ_left (m n : Set) :
  ...
  · intro k hkω hIh
    ...
    have hmksuccω : m * k⁺ ∈ ω := nat_mul_closed m (k⁺) hmω (thm_4B_ω_inductive.right k hkω)
    calc
...
abbrev nat_mul_succ_left := thm_4K_mul_succ_left

/- [Enderton, Theorem 4K (component: commutativity of multiplication), pp.82-83] -/
theorem thm_4K_mul_comm (a b : Set) :
  a ∈ ω → b ∈ ω → a * b = b * a := by
  intro haω hbω
  apply ω_induction (fun k => a * k = k * a)
  · change a * zero_ω = zero_ω * a
  ...
  · exact hbω
abbrev nat_mul_comm := thm_4K_mul_comm

/- [Enderton, Theorem 4K (component: right distributivity), pp.82-83] -/
theorem thm_4K_right_distrib (a b c : Set) :
...
abbrev nat_right_distrib := thm_4K_right_distrib

...
theorem equiv_class_eq_iff (R A x y : Set) :
  ...
  · intro hClass
    ...
    have hyRan : y ∈ (ran R) := Relation.Pair.mem_ran R y y hyy
...
lemma compatible_map_eqclass (R A F x y u v : Set) :
  ...
  have huA : u ∈ A := hRanSubA u (Relation.Pair.mem_ran F x u hxuF)
  have hvA : v ∈ A := hRanSubA v (Relation.Pair.mem_ran F y v hyvF)
...
lemma thm_3Q_equivalence_class_mem_quotient (A R x : Set) :
    x ∈ A → [x]₍R₎ ∈ A / R := by
  intro hxA
  rw [QuotientSet.Spec]
  refine ⟨?_, x, hxA, rfl⟩
...
lemma EquivalenceClass.mem_quotient (A R x : Set) :
    x ∈ A → [x]₍R₎ ∈ A / R :=
  thm_3Q_equivalence_class_mem_quotient A R x
...
lemma QuotientLift.pair_intro (R A F x y : Set) (hMap : MapsInto F A A) :
  ...
  intro hxA hxyF
  rcases hMap with ⟨_, _, hRanSubA⟩
  have hyA : y ∈ A := hRanSubA y (Relation.Pair.mem_ran F x y hxyF)
  have hxQ : [x]₍R₎ ∈ A / R := thm_3Q_equivalence_class_mem_quotient A R x hxA
  have hyQ : [y]₍R₎ ∈ A / R := thm_3Q_equivalence_class_mem_quotient A R y hyA
  have hProd : ⟪[x]₍R₎, [y]₍R₎⟫ ∈ ((A / R) ⨯ (A / R)) :=
    (Product.Pair.Spec).2 ⟨hxQ, hyQ⟩
...
theorem quotient_function_exists
  ...
  have hDomFq : (dom Fq) = A / R := by
    ...
    · intro hQ
      ...
      exact (Domain.Spec).2 ⟨[y]₍R₎, hQPair⟩
...
theorem equiv_classes_partition (R A : Set) :
  ...
  · intro B hB
    ...
    · have hxx : ⟪x, x⟫ ∈ R := hRefl x hxA
      have hxRan : x ∈ (ran R) := Relation.Pair.mem_ran R x x hxx

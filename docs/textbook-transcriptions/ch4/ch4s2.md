...
Source: Enderton, Chapter 4 "Peano's Postulates" and transitive-set material, pp. 70–73 in `135 Elements of Set Theory - Enderton.pdf`.
...
Enderton states Peano postulates with `S` as a function `N → N`. This repo uses a **Lean-level** successor `S : Set → Set` and packages:

- `e ∈ N`, `∀ n ∈ N, S n ∈ N`, `∀ n ∈ N, S n ≠ e`, injectivity of `S` on `N`, and the subset induction principle.

This matches the book’s content; `e ∉ ran S` is expressed as “no successor hits `e`.”
...
- **4D:** `0 ∈ ω` and successor closure from `ω` inductive; `n⁺ ≠ 0` via `successor_ne_empty`; injectivity of successor on `ω` via 4F + 4E and `⋃(m⁺) = m`; induction postulate from minimality of `ω` (`ω.subset_of_inductive`).
